...
if(@$_GET['action'] == 'delete' && isset($_POST['ids'])){
    ...
    if($_rows2 = _fetch_array("SELECT user_id FROM tb_user WHERE user_username='{$_COOKIE['username']}' LIMIT 1")) {
        ...
        //执行删除短信的操作
        _query("DELETE FROM tb_message WHERE m_id IN ({$_clean['ids']})");
        if(_affected_rows()){
            //关闭数据库链接
            _close();
//            _session_destroy();
...
_page("SELECT m_id FROM tb_message WHERE m_touser='{$_COOKIE['username']}'",15);//第一个参数是获得总的数据，第二个参数是每页显示的数据量
$_result = _query("SELECT m_id,m_fromuser,m_content,m_state,m_date FROM tb_message WHERE m_touser='{$_COOKIE['username']}' ORDER BY m_date DESC LIMIT $_pagenum,$_pagesize");
            ...
            <?php
                ...
                while($_rows = _fetch_array_list($_result)){
                    ...
                    if (empty($_rows['m_state'])){
                        ...
                        $_html['content_xh'] = "<strong>"._title($_html['content'],14)."</strong>";
                    }else{
                        $_html['state'] = "<img src='images/read.png' alt='已读' title='已读'>";
                        $_html['content_xh'] = _title($_html['content'],14);
                    }
            ?>
            <tr>
                <td><?php echo $_html['fromuser'] ?></td>
                <td><a href="member_message_detail.php?id=<?php echo $_html['id']?>" title="<?php echo $_html['content'] ?>"><?php echo $_html['content_xh'] ?></a></td>
